...
function getMigrationsCollection(db, mongodb, migrationCollection) {

	// set default if the migration collection is undefined
    if (typeof migrationCollection === 'undefined') {
        migrationCollection = 'migrations';
    }

	// for mongodb 2.x
	if (typeof db.collection !== 'undefined') {
		return db.collection(migrationCollection);
	}

	return new mongodb.Collection(db, migrationCollection);
}
...
function getConnection(opts, cb) {
	...
	if (opts.connectionString) {
		...
		MongoClient.connect(opts.connectionString, function(err, db) {
			...
			cb(null, {
				connection: db,
				migrationCollection: getMigrationsCollection(db, mongodb, opts.migrationCollection)
			});
		});
		return;
	}
